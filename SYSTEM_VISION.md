...
| O9 | ¿Debe la etiqueta del muñeco llevar **dos líneas** —un nombre y el repositorio— en lugar de una? Pedido el 4/8/2026. **Restricción encontrada:** el título de la conversación **no se puede usar**, ni el que genera Claude ni el que pongas con `/rename`: no viaja en los hooks, y la única forma de leerlo sería abrir el fichero de transcripción, que es exactamente lo que **D5** prohíbe. La alternativa sin coste de privacidad es que el nombre lo ponga el dueño editando la tarea. Pendiente además de resolver el espacio: la etiqueta mide 96 px y una línea, y en el cambio #12 ya hubo que elegir entre dos datos porque no caben | Dueño del proyecto | Cuando la planta con varias conversaciones se vea en el uso real y se sepa si el código de sesión basta |
| O10 | ¿Debe la Torre **abrir** la conversación que termina o te reclama? **Restricción encontrada:** `claude --resume <código>` funciona y la Torre tiene los datos, pero abre una **terminal nueva**, no devuelve el foco a la ventana de VSCode donde está la conversación; no hay enlace profundo documentado que lo permita. Así que lo que se puede construir no resuelve el caso real. Abrirlas solas, además, supondría varias ventanas robando el foco y que la Torre ejecute procesos por su cuenta —otra decisión que reabrir— | Dueño del proyecto | Si aparece una vía para saltar a la sesión del editor, o si el uso real demuestra que una terminal nueva sirve |
