...
import type { PendingTurn } from './turns.js'
...
export const IPC = {
  ...
  /** Main → renderer: la lista de turnos esperando respuesta ha cambiado. */
  turnsChanged: 'turns:changed',
} as const
...
export interface TorreBridge {
  ...
  /** Suscripción a los turnos que esperan tu respuesta (D25). */
  onTurnsChanged: (listener: (pending: PendingTurn[]) => void) => () => void
}
